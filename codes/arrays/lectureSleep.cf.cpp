...
void solve(){
    ...
    _input(w, 0, n);
    vi prefTotal(n+1, 0);
    vi prefWithSleep(n+1, 0);
    rep(i, 1, n+1){
        prefTotal[i] = prefTotal[i-1] + a[i-1];
        prefWithSleep[i] = prefWithSleep[i-1] + (w[i-1] == 1?a[i-1]:0LL);
    }
    int ans = prefWithSleep[n];
    rep(i ,1, n+1){
        if(w[i-1] == 0){
            int idx = min(n, i+k-1);
            int cumm = prefWithSleep[i-1] + (prefTotal[idx] - prefTotal[i-1]) + prefWithSleep[n] - prefWithSleep[idx];
            ans = max(ans, cumm);
        }
    }
    cout<<ans<<endl;
